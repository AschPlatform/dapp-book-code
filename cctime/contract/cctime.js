...
module.exports = {
  postArticle: async function(title, url, text, tags, cid, settleHeight) {
    ...
    if (settleHeight <= this.block.height + constants.SETTLE_BLOCK_PERIOD) {
      // TODO
      return 'Settlement period between  24 hours and 1 month'
      // settleHeight <= this.block.height + constants.SETTLE_BLOCK_PERIOD
    }
  ...
  voteArticle: async function(aid, amount) {
    ...
    let { awardType = constants.AWARD_TYPE.DEFAULT, cid } = article
    let res
    switch (awardType) {
